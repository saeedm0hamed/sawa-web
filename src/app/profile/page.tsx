    ...
    <>
      <div className='min-h-screen flex flex-col relative z-0'>
        ...
        <div className='absolute inset-0 -z-10'>
          ...
          />
          <div className='absolute inset-0 bg-black/40' style={{ backdropFilter: 'blur(200px)' }} />
        </div>
        ...
        ${showAlert ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5 pointer-events-none'}`}
          >
            <Alert variant='default' className='w-fit mx-auto shadow-lg overflow-hidden flex items-center gap-2'>
              <CheckCircle />
              <AlertTitle className='text-foreground w-full'>تم إرسال رابط التفعيل إلى إيميلك</AlertTitle>
            </Alert>
          </div>
          ...
          <div className='flex flex-col gap-3 md:gap-5'>
            {profile.recent_views && profile.recent_views.length >= 1 && (
              <SectionSlider title='السجل' data={[...profile.recent_views].reverse()} />
            )}

            {profile.favorites && profile.favorites.length >= 1 && (
              <SectionSlider title='قائمة المفضلة' data={profile.favorites} />
            )}
            {favGenersData && <SectionSlider title='الأقتراحات' data={favGenersData} />}
          </div>
        </main>
        <Footer />
      </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className='bg-[#1a1a1a] w-[95%] max-w-md rounded-2xl px-4 md:px-8 py-8 shadow-2xl border border-white/10'>
          <DialogTitle className='text-xl font-bold text-center'>تعديل البيانات الشخصية</DialogTitle>

          <div className='flex flex-col gap-4'>
            <p className='text-sm text-white/60 text-center -mt-2'>عدل بياناتك واحفظ التغييرات</p>
      ...
      <Dialog open={logoutConfirmOpen} onOpenChange={setLogoutConfirmOpen}>
        <DialogContent className='bg-[#1a1a1a] w-[95%] max-w-md rounded-2xl px-6 py-8 shadow-2xl border border-white/10'>
          <DialogTitle className='text-xl font-bold text-center'>هل انت متأكد من تسجيل الخروج</DialogTitle>
          <div className='flex justify-between gap-2'>
            ...
            <Button onClick={handleLogout} className='flex-1 rounded-xl bg-red-500 hover:bg-red-500/60'>
              تأكيد الخروج
            </Button>
          </div>
        </DialogContent>
